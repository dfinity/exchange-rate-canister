...
impl IsForex for BankOfIsrael {
    ...
    fn extract_rate(&self, bytes: &[u8], timestamp: u64) -> Result<ForexRateMap, ExtractError> {
        ...
        let data: XmlBankOfIsraelCurrencies =
            serde_xml_rs::from_str(&String::from_utf8(bytes.to_vec()).unwrap()).unwrap();
        ...
        if extracted_timestamp != timestamp {
            ...
            let values = values
                .iter()
                .filter_map(|item| Some((item.currencycode.to_lowercase(), (item.rate * 10_000.0) as u64 / item.unit)))
                .collect::<ForexRateMap>();
            if extracted_timestamp == timestamp {
                self.normalize_to_usd(&values)
            } else {
...
mod test {
    ...
    fn query_string_test() {
        ...
        assert_eq!(query_string, "https://www.boi.org.il/currency.xml?rdate=20220826");
    }
    ...
    #[test]
    fn extract_rate_from_israel_test() {
        let israel = BankOfIsrael;
