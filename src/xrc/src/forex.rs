...
use crate::{
    median, standard_deviation, utils, AllocatedBytes, ExtractError, QueriedExchangeRate, ONE_DAY,
    ONE_HOUR, ONE_KIB, RATE_UNIT, USD,
};
...
impl ForexRateStore {
    ...
    fn shift_to_latest_source_eod(requested_timestamp: u64, current_timestamp: u64) -> u64 {
        ...
        let shift_to_latest_source_eod =
            (ONE_DAY as i64 + (max_shift_hours * ONE_HOUR as i64)) as u64;
        ...
        if current_timestamp < requested_day_end_on_all_sources {
            requested_timestamp.saturating_sub(ONE_DAY)
        } else {
            requested_timestamp
        }
    }
    ...
    pub(crate) fn get(
        ...
        let mut requested_timestamp = (requested_timestamp / ONE_DAY) * ONE_DAY;
        ...
        while go_back_days <= MAX_DAYS_TO_GO_BACK {
            let query_timestamp = requested_timestamp.saturating_sub(ONE_DAY * go_back_days);
            go_back_days += 1;
...
trait IsForex {
    ...
    fn offset_timestamp_to_timezone(&self, current_timestamp: u64) -> u64 {
        (current_timestamp as i64 + (self.get_utc_offset() as i64 * ONE_HOUR as i64)) as u64
    }
...
mod test {
    ...
    fn rate_collector_update_and_get() {
        ...
        let first_day_timestamp = (123456789 / ONE_DAY) * ONE_DAY;
        let rates = hashmap! {
            "EUR".to_string() => 1_000_000_000,
            "SGD".to_string() => 100_000_000,
            "CHF".to_string() => 700_000_000,
        ...
        // Add a new day
        let second_day_timestamp = first_day_timestamp + ONE_DAY;
        ...
        let third_day_timestamp = second_day_timestamp + ONE_DAY;
    ...
    fn ensure_rate_store_removes_usd_rates() {
        ...
        // Start by executing the same logic as for the [OneDayRatesCollector] to verify that the calls are relayed correctly.
        let timestamp = (123456789 / ONE_DAY) * ONE_DAY;
        collector.update(
            "src1".to_string(),
            timestamp,
            hashmap! {
    ...
    fn rate_store_update() {
        ...
        assert!(
            matches!(result, Err(GetForexRateError::CouldNotFindBaseAsset(timestamp, ref asset)) if timestamp == (1234 / ONE_DAY) * ONE_DAY && asset == "HKD"),
    ...
    fn rate_store_gets_rate_in_past_if_current_day_is_not_over() {
            ...
            hashmap! {
                "EUR".to_string() =>
                    QueriedExchangeRate {
                        ...
                        quote_asset_num_queried_sources: 5,
                        quote_asset_num_received_rates: 5,
                        forex_timestamp: Some(ONE_DAY * 2),
                    },
            },
        );
        ...
        let result = store.get(ONE_DAY / 2, ONE_DAY, "EUR", USD);
        assert!(matches!(
            result,
        ...
        let result = store.get(ONE_DAY, ONE_DAY * 2 + ONE_DAY / 2, "EUR", USD);
        assert!(matches!(
            result,
            Ok(rate) if rate.rates == vec![1_000_000_000] && rate.base_asset_num_received_rates == 5,
        ));
        ...
        let result = store.get(ONE_DAY * 2, ONE_DAY * 2 + ONE_DAY / 2, "EUR", USD);
        assert!(matches!(
            result,
            Ok(rate) if rate.rates == vec![1_000_000_000] && rate.base_asset_num_received_rates == 5,
        ));
        ...
        let result = store.get(ONE_DAY * 2, ONE_DAY * 4, "EUR", USD);
        assert!(matches!(
            result,
        ...
        let result = store.get(ONE_DAY * 3, ONE_DAY * 3 + ONE_DAY / 2, "EUR", USD);
        assert!(matches!(
            result,
            Ok(rate) if rate.rates == vec![1_500_000_000] && rate.base_asset_num_received_rates == 5,
        ));
    ...
    fn forex_go_back_days() {
        ...
        let queried_timestamp = timestamp + ONE_DAY * MAX_DAYS_TO_GO_BACK;
        ...
        let queried_timestamp = queried_timestamp + ONE_DAY * 2 + ONE_DAY / 2;
        assert!(matches!(
