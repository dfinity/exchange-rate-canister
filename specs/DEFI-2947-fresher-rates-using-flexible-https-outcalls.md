...
- **Second-granular observation time** (R10) — honest freshness reporting; differs from settled's minute granularity. Expectation-setting: "forming minute" is the **query target**, not the delivered latency. A live rate is bounded by the outcall round trip (~10–15s) plus up to `T` (10s) of cache age, so it is typically **~10–25s old on delivery** — far fresher than settled's ~1–2.5 min, but not sub-second. The reported `timestamp` (R10) always states the true observation age so callers are never misled about it.
