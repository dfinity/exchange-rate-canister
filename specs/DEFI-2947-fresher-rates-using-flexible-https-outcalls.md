...
- **R4** — `Live` is served only for pairs built from the dedicated live allowlist `LIVE_ALLOWLIST = [BTC, ETH, ICP, USDC]` (each with deep forming-minute liquidity on 6+ exchanges), with `USDT` permitted **only as the bridge/quote asset**. Concretely, the **base must be in `LIVE_ALLOWLIST`** and the **quote must be in `LIVE_ALLOWLIST` or `USDT`** — so `USDT` is only ever the quote/bridge, never a live-priced base: this yields the single-leg pairs `{BTC,ETH,ICP,USDC}/USDT` plus the cross pairs among allowlisted assets (2 legs, e.g. `BTC/USDC`). Any other asset — **including all fiat** — makes a `Live` request return an **error** (`ExchangeRateError::Other` with a dedicated code), not a silent `Settled` downgrade. `LIVE_ALLOWLIST` is a **new constant** (a subset of `PRIVILEGED_CRYPTO_ASSETS`); it must **not** be expressed by mutating `PRIVILEGED_CRYPTO_ASSETS`, which feeds settled rate-limit-bypass logic (`utils::is_privileged_asset_pair`) that must stay unchanged (R2). Because fiat is never allowlisted, a `Live` pair never contains a fiat leg (a fiat conversion would still be settled — R16).
...
- **R15** — Flexible outcalls cost XRC nothing on its `Free`-cost-schedule subnet, and the caller-facing `get_exchange_rate` cycles fee is unchanged. Flexible outcalls are priced under **PAYG** (`PricingVersion::PayAsYouGo`, which the replica sets for every flexible request). PAYG charges **actual usage, not the requested maximum** — per replica `50·actual_downloaded_bytes + 300·ms + 50·actual_transformed_bytes·N + transform_instr/13`, and its up-front base fee for a flexible request has **no response-size term** — and it **short-circuits to zero on a `Free` cost schedule** (the `is_free` path refunds the full allowance). Consequently `max_response_bytes` is **not** a pricing input for flexible outcalls (see Constraints), and on XRC's `Free` subnet the outcall is free regardless of response size. (`PricingVersion` and the PAYG/legacy pricing code are management-canister/replica code in the **`dfinity/ic`** repo, not XRC — see Constraints.) Initial availability is **free-subnet-only**: flexible outcalls are enabled on `Free`-cost-schedule subnets only for now, so on a `Normal` subnet the feature is unavailable (the failure mode is "unavailable", never "unexpectedly charged").
...
- **Committee quorum is a tunable compile-time constant, not a runtime knob.** `min_responses` (the fraction of committee nodes that must return an OK response before an exchange counts) is a named constant, adjustable only by rebuild + NNS proposal — consistent with XRC having no out-of-band runtime control (R12, and the shadow-compute constants). The default `min_responses = ⌈2N/3⌉+1` (= 24 on XRC's production N = 34 subnet; clamped to ≤ N for safety) favors **trust** (hard to skew the median with a minority of nodes) at the cost of **availability**: it is precisely the popular, high-liquidity endpoints most likely to rate-limit (429/403) part of the committee, dropping OK responses below quorum and yielding an error. Lowering `min_responses` trades trust for availability. It stays a static constant (no dynamic adjustment) so behavior is deterministic across replicas and reviewable in a proposal; retune it from shadow data if the calm-market inconsistency rate is too high. **Committee sizing (`total_requests`) is a separate decision:** because the replica **rejects** `total_requests > N` (it does not clamp — see Constraints), an explicit `total_requests = 34` is fragile to subnet-topology shrink. PR3 chooses between passing `replication = None` — the replica defaults the committee to the live `N` with a floor-based `⌊2N/3⌋+1` quorum (robust to topology, but the quorum is then untuned) — and an explicit `Some { … }` with a safety margin (keeps the tuned `⌈2N/3⌉+1` quorum). The `⌈2N/3⌉+1` figure above is the target only if we keep explicit control.
...
- **Second-granular observation time** (R10) — honest freshness reporting; differs from settled's minute granularity. Expectation-setting: "forming minute" is the **query target**, not the delivered latency. The observation (R10) is taken when the outcalls resolve — near the *end* of the round trip — so a freshly-fetched rate's staleness on delivery is mainly the return-network leg (a few seconds), plus up to `T` (10s) for a cache hit: an **upper bound of ~25s**, versus settled's ~1–2.5 min. (The caller's full request round trip is ~10–15s, but the number is observed near its end, so it is fresher than that figure suggests.) Far fresher than settled, but not sub-second; the reported `timestamp` (R10) always states the true observation age, so callers are never misled.
...
- The replica `flexible_http_request` API — with `get_own_cost_schedule`, `PricingVersion`, and the PAYG path referenced in R15 — lives in the **`dfinity/ic`** repo (management canister / replica), **not** here, and XRC has **no build-time dependency on it**. XRC builds against the published `ic-cdk` crate (`0.19.0`); since there is no ic-cdk helper for `flexible_http_request`, XRC **hand-rolls the flexible Candid types and calls via `call_raw`** (see Implementation). So the dependency is not at build time but (a) **runtime** — the replica on XRC's subnet must have flexible outcalls enabled (critical-path gate below) — and (b) **interface stability** — XRC's hand-encoded Candid must match the final field layout. The API is not yet in the public `ic.did`, currently sits on `dfinity/ic` `eichhorl/*` branch(es), and its layout may still shift; it is expected to merge to `dfinity/ic` `master` the **week of 2026-07-20**. The layout has been located and the hand-rolled types mirrored from branch `eichhorl/flexible-max-response-bytes` (`rs/types/management_canister_types/src/http.rs`); track it as it stabilizes and adjust the hand-rolled types if it changes; no rebuild of XRC *against `ic`* is involved.
- A flexible request = **1** in-flight outcall slot regardless of committee size (subnet cap 3000). Flexible outcalls are priced under **PAYG**, which charges **actual** usage (downloaded + transformed bytes, round-trip time, transform instructions) and short-circuits to **zero on a `Free` cost schedule**. XRC's subnet is `Free` (inferred from XRC attaching 0 cycles today and working), so flexible is free there; flexible is enabled on `Free`-cost-schedule subnets only for now ⇒ the feature is **initially free-subnet-only**. Verified against `dfinity/ic` branch `eichhorl/flexible-max-response-bytes` (`rs/https_outcalls/pricing/{payg,legacy}.rs`; `rs/cycles_account_manager` `http_request_base_fee`, whose `Flexible` arm has no response-size term).
- **`max_response_bytes` is optional and not a pricing input for flexible outcalls.** The flexible args carry `max_response_bytes : opt nat64`; `None` ⇒ the replica's 2 MiB ceiling (`MAX_CANISTER_HTTP_RESPONSE_BYTES = 2,000,000`). Under PAYG it is used **only** as the adapter's per-node download ceiling (an over-large response aborts ⇒ `responses_too_large`), never as the charge basis — contrast the classic replicated/legacy fee, which scales with the requested max (the reason `settled` pins a tight 3 KiB). So `live` leaves it unset (or a single generous constant); a tight per-exchange cap would only risk spurious `responses_too_large` for no cost benefit. The flexible args have **no `pricing_version` field** (unlike the classic args); the replica selects PAYG itself.
- **Committee-count validation is a hard reject, not a clamp.** The replica's `generate_from_flexible_args` requires `1 ≤ total_requests ≤ N` (available nodes) and `min_responses ≤ max_responses ≤ total_requests`; `total_requests > N` is **rejected** as `invalid_parameters` (`InvalidReplicationCounts`). So a hardcoded `total_requests = 34` breaks every live outcall if XRC's subnet ever drops below 34 nodes. Passing `replication = None` instead makes the replica default the committee to the **live** `N` — `(N, ⌊2N/3⌋+1, N)` — robust to topology at the cost of the tuned quorum (the default is floor-based `⌊2N/3⌋+1 = 23` at N = 34, vs our ceil-based `24`). Choose `None` vs. explicit-with-margin in PR3 (see Design Decisions).
...
- **Production subnet:** XRC (`uf6dk-hyaaa-aaaaq-qaaaq-cai`) runs on the system subnet `uzr34-akd3s-xrdag-3ql62-ocgoh-ld2ao-tamcv-54e7j-krwgb-2gm4z-oqe` with **N = 34** nodes ⇒ committee `total_requests = 34`, `min_responses = ⌈2·34/3⌉+1 = 24`, `max_responses = 34`. (If `replication = None` is chosen instead, the replica derives these from the live `N` with a floor-based quorum; see the committee-validation constraint above.)
...
- Build `FlexibleCanisterHttpRequestArgs { url, max_response_bytes: None, headers, body: None, method: GET, transform, replication }` (field layout verified against `dfinity/ic` `eichhorl/flexible-max-response-bytes`; `max_response_bytes` left `None` per Constraints); `call_raw(management_canister, "flexible_http_request", …)`; decode `FlexibleHttpRequestResult`. `replication` is either `None` (replica defaults the committee to the live `N`) or `Some(ReplicationCounts { total_requests, min_responses = MIN_COMMITTEE_RESPONSES, max_responses })` — see the committee-validation constraint and Design Decisions for the trade-off. `MIN_COMMITTEE_RESPONSES` (default `⌈2N/3⌉+1`, clamped to ≤ N; = 24 at the production N = 34) is a compile-time constant — the tunable trust/availability knob, changed only by rebuild + proposal.
- Result shape: `FlexibleHttpRequestResult = variant { ok : vec CanisterHttpResponsePayload; err : FlexibleHttpRequestErr }`. Map `ok` → the per-node response vector (aggregation is PR3). `FlexibleHttpRequestErr = record { global_error : opt variant { invalid_parameters; timeout; out_of_cycles; responses_too_large; too_many_rejects }; node_details : vec …; message : text }` — `global_error` is **optional** (an `err` may carry only per-node `node_details` with `global_error = None`), so error mapping must handle that case. Per-node identity (`node_id`, per-node error) exists **only** in `node_details` on the `err` path; the `ok` vector is anonymous (no `node_id` per response) — this constrains the per-node/committee metrics (see Metrics).
- Cycle computation (no helper); on a `Free` subnet attaching 0 works, mirroring `Exchange::cycles()` (PAYG charges actual usage and is free on `Free`; see R15/Constraints).

### Exchange ticker endpoints (`src/xrc/src/exchanges.rs`)

- Per exchange, add a real-time ticker URL + `extract_rate`-style parser used **only** in `live` mode; leave the candle endpoint/parser for `settled` untouched. Prefer a **per-symbol** ticker endpoint where the exchange offers one (smaller payload) over an all-symbols ticker. Add `test-data/exchanges/<exchange>-ticker.json` fixtures, captured **as-received** (do not minify — some exchanges, e.g. crypto.com/Poloniex, serve pretty-printed JSON, and minifying would understate the real byte size).
- Leave `max_response_bytes` unset (`None` ⇒ the replica's 2 MiB ceiling) or set a single generous constant — deliberately unlike settled's cost-tuned 3 KiB (see Constraints / R15: flexible/PAYG charges actual bytes, free on XRC's subnet; a tight cap only risks spurious `responses_too_large`).
- Add a test asserting each captured `<exchange>-ticker.json` fixture is comfortably under `MAX_CANISTER_HTTP_RESPONSE_BYTES` (2 MiB / 2,000,000 bytes), so a real ticker response fits the IC's maximum HTTP-outcall response size; also verify this when downloading/adding a fixture.
- **Live transform method.** The flexible transform func has signature `func (record { response : http_response; context : blob }) -> (http_response) query` where `http_response.status` is `nat` — candid-compatible with the classic ic-cdk `HttpResponse` (`status : candid::Nat`, also `nat`). Register a **live-specific** transform query method (reducing each ticker response to the extracted rate, R11) rather than assuming the settled `transform_exchange_http_response` drops in unchanged (it parses candle payloads).
...
Note: on a successful (`ok`) flexible response the per-node vector is **anonymous** (no `node_id`), so `xrc_live_committee_responses` counts OK responses per exchange from the `ok` vector length and derives reject/timeout outcomes (with node-level detail) from the `err` path's `node_details`; full per-node attribution is only available when the outcall returns `err` (see Flexible outcall binding).
...
- **PR1 — Flexible outcall binding** — `flexible_http.rs` + unit tests (arg construction, cycle calc, result/error decoding). R11; binding for R3/R15.
- **PR2 — Ticker endpoints** — per-exchange ticker URL + parser (feature-gated) + fixtures (with a test that each fixture is well under 2 MiB) + a live-specific transform method + parser/URL tests; `max_response_bytes` left unset/generous. Settled untouched (R2).
- **PR3 — Aggregation & spread** — committee-median-per-exchange feeding the cross-exchange median; spread→`InconsistentRatesReceived`. R3, R9.
- **PR4 — Cache, coalescing, budget** — live cache (T=10s, `fetched_at`), per-leg inflight coalescing, per-pair admission (K=5), separate `LIVE_REQUEST_COUNTER_LIMIT`. R6, R7, R8.
- **PR5 — Shadow-compute + metrics + circuit breaker** — compute `live` alongside sampled `settled` traffic without returning it; all new metrics + request-log/dashboard `freshness` column; enable + sample rate as compile-time constants; the self-recovering autonomous circuit breaker. No public API surface. R13, R14, R17.
...
- **Replica enablement (critical path):** flexible outcalls must be enabled by the replica on XRC's production system subnet (`uzr34-…`, 34 nodes). Until then no real flexible outcall works anywhere available to us (`beta` is not on a system subnet), and shadow-compute cannot gather data.
- **Cost-schedule gate:** confirm — against the live registry, not assumed — that XRC's subnet `uzr34-…` (hosting `uf6dk-hyaaa-aaaaq-qaaaq-cai`) is `Free` cost schedule. On `Free`, flexible is free *and* available (PAYG short-circuits `charge()` to zero); flexible is enabled on `Free`-cost-schedule subnets only for now, so on a `Normal`-schedule subnet the feature is unavailable.
