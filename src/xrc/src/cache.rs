...
impl ExchangeRateCache {
    ...
    #[allow(dead_code)]
    pub(crate) fn insert(&mut self, rate: QueriedExchangeRate, time: u64, retention_period: u64) {
        // Make sure that the quote asset is correct.
        if rate.quote_asset != self.get_quote_asset() {
            panic!("Invalid quote asset: {}", rate.quote_asset.symbol);
        }
...
mod test {
    ...
    fn get_basic_rate() -> QueriedExchangeRate {
        ...
        QueriedExchangeRate::new(base_asset, quote_asset, 100, &[1_230_000], 1, 1, None)
    }
    ...
    #[test]
    #[should_panic(expected = "Invalid quote asset: USDC")]
    ...
    fn verify_cache_insert() {
        ...
        cache.insert(basic_rate.clone(), 150, retention_period_s);
        ...
        cache.insert(rate, 150, retention_period_s);
        assert_eq!(cache.size(), 2);

        // Adding the first rate again at a different time replaces the first entry.
        cache.insert(basic_rate.clone(), 160, retention_period_s);
        assert_eq!(cache.size(), 2);
        ...
        let mut rate = basic_rate.clone();
        rate.timestamp = 210;
        cache.insert(rate, 210, retention_period_s);
        assert_eq!(cache.size(), 2);
        let cached_rate = &cache.rates.get("ICP").unwrap()[1];
        ...
        rate.timestamp = 220;
        cache.insert(rate, 220, retention_period_s);
        assert_eq!(cache.size(), 2);
        let cached_rate = &cache.rates.get("ICP").unwrap()[1];
    ...
    fn verify_cache_get() {
        ...
        cache.insert(btc_rate, 160, retention_period_s);
        ...
        cache.insert(icp_rate, 190, retention_period_s);
    ...
    fn verify_cache_pruning() {
        ...
        cache.insert(rate.clone(), 100, retention_period_s);
        ...
        cache.insert(rate.clone(), 100, retention_period_s);
        rate.timestamp = 160;
        cache.insert(rate, 100, retention_period_s);
    ...
    fn verify_variable_retention_periods() {
        let mut cache = ExchangeRateCache::new(USDT.to_string(), 3, 5);
        let mut rate = get_basic_rate();
        cache.insert(rate.clone(), 100, 20);

        rate.timestamp = 110;
        cache.insert(rate.clone(), 110, 90);
        assert_eq!(cache.size(), 2);

        rate.timestamp = 120;
        // The first entry should be removed.
        cache.insert(rate.clone(), 120, 40);
        assert_eq!(cache.size(), 2);

        rate.timestamp = 150;
        cache.insert(rate.clone(), 150, 10);
        assert_eq!(cache.size(), 3);
